        ...
        fun getTimeRequest(context: Context, queue: RequestQueue, data: HashMap<String, Any>): Boolean {
            ...
            val responseListener = Response.Listener<String?> { response ->
                try {
                    ...
                    val success = jsonObject.getBoolean("success")
                    if (success) {
                        Toast.makeText(context, jsonObject.getString("totaltime").toString(), Toast.LENGTH_SHORT).show()
                        AppStat.myStat.settotalExerciseTime(jsonObject.getString("totaltime").toInt())
                    } else {
                        ret = false
                        return@Listener
                    }
